...
// ── useTweaks ───────────────────────────────────────────────────────────────
// Single source of truth for tweak values. setTweak persists via localStorage
function useTweaks(defaults) {
  const [values, setValues] = React.useState(() => {
    try {
      const saved = localStorage.getItem('plot-tweaks');
      if (saved) return JSON.parse(saved);
    } catch (e) {
      console.warn("Failed to load tweaks from localStorage", e);
    }
    return defaults;
  });
  // Accepts either setTweak('key', value) or setTweak({ key: value, ... }) so a
  // useState-style call doesn't write a "[object Object]" key.
  const setTweak = React.useCallback((keyOrEdits, val) => {
    const edits = typeof keyOrEdits === 'object' && keyOrEdits !== null
      ? keyOrEdits : { [keyOrEdits]: val };
    setValues((prev) => {
      const next = { ...prev, ...edits };
      localStorage.setItem('plot-tweaks', JSON.stringify(next));
      return next;
    });
